...
function openFilePickerWeb(options: { multiple?: boolean; directory?: boolean } = {}): Promise<File[]> {
...
export function ChatInput({
  ...
  const handleAttachFiles = useCallback(async () => {
    ...
    const files = await openFilePickerWeb({ multiple: true });
    if (files.length > 0) {
      await addBrowserFiles(files);
    }
  }, [addBrowserFiles, disabled]);

  const handleAttachFolders = useCallback(async () => {
    if (disabled) {
      return;
    }
    const files = await openFilePickerWeb({ multiple: true, directory: true });
    if (files.length > 0) {
      await addBrowserFiles(files);
    }
  }, [addBrowserFiles, disabled]);
