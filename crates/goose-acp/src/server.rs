...
impl GooseAcpAgent {
    ...
    async fn on_add_config_extension(
        ...
        let config: goose::agents::ExtensionConfig =
            serde_json::from_value(serde_json::Value::Object(obj))
                .map_err(|e| sacp::Error::invalid_params().data(format!("bad config: {e}")))?;

        goose::config::extensions::set_extension(goose::config::extensions::ExtensionEntry {
            enabled: req.enabled,
            config,
        });
        Ok(EmptyResponse {})
    ...
    async fn on_toggle_config_extension(
        ...
        goose::config::extensions::set_extension_enabled(&req.config_key, req.enabled);
        Ok(EmptyResponse {})
    }
